...
class TestWebSearch:
    ...
    async def test_buscar_web_sin_api_key(self, mock_settings):
        """Test de búsqueda web sin API key - debería fallar."""
        mock_settings.SEARCH_API_KEY = ""
        
        with pytest.raises(WebSearchError, match="Búsqueda web no configurada"):
            await buscar_web("test query", mock_settings)
    
    @pytest.mark.asyncio
    async def test_buscar_web_error_http(self, mock_settings):
        """Test de error HTTP en búsqueda web - debería fallar."""
        
        with patch('httpx.AsyncClient') as mock_client:
            # Configurar mock para que Bing falle
            mock_response = MagicMock()
            ...
            with pytest.raises(WebSearchError, match="Búsqueda web no disponible"):
                await buscar_web("test query", mock_settings)
