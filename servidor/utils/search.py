...
async def buscar_web(query: str, settings: Settings, top: int = None) -> List[Dict[str, str]]:
    ...
    if not settings.SEARCH_API_KEY:
        raise WebSearchError("SEARCH_API_KEY no configurada")
    ...
    try:
        ...
        logger.error(f"Error HTTP en búsqueda web: {e.response.status_code} - {e.response.text}")
        raise WebSearchError(f"Error en la API de búsqueda: {e.response.status_code}")
    except httpx.TimeoutException:
        logger.error(f"Timeout en búsqueda web para query: {query}")
        raise WebSearchError("Timeout en la búsqueda web")
    except Exception as e:
        logger.error(f"Error inesperado en búsqueda web: {str(e)}")
